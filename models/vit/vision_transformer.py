...
__all__ = ['VisionTransformer', 'WindowVisionTransformer']  # model_registry will add each entrypoint fn to this
...
"""
    The block applies temporal tokens.
"""
class SeparateTemporalBlock(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        B = x.size(0)
        temporal_token = x[:, :self.window_size, :]
        ...
        temporal_token = rearrange(temporal_token, 'b w m -> (b w) m').unsqueeze(1)
        x = torch.cat([temporal_token, sequence_token], dim=1)
        ...
        temporal_token = x[:, 0, :]
        temporal_token = rearrange(temporal_token, '(b w) m -> b w m', b=B)
        x = torch.cat([temporal_token, sequence_token], dim=1)
        
        return x


class SeparateTemporalBlockAlpha(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sequence_token = x[:, self.window_size: , :]
        sequence_token = sequence_token + self.drop_path1(self.ls1(self.attn(self.norm1(sequence_token))))
        sequence_token = rearrange(sequence_token, 'b (n w) d -> (b w) n d', b=B, w=self.window_size)
        temporal_token = rearrange(temporal_token, 'b w d -> (b w) d').unsqueeze(1)
        x = torch.cat([temporal_token, sequence_token], dim=1)
        x = x + self.alpha * self.temporal_attn(self.temporal_norm1(x))
        sequence_token = x[:, 1:, :]
        sequence_token = rearrange(sequence_token, '(b w) n d -> b (n w) d', b=B, w=self.window_size)
        temporal_token = x[:, 0, :]
        temporal_token = rearrange(temporal_token, '(b w) d -> b w d', b=B)
        x = torch.cat([temporal_token, sequence_token], dim=1)
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        return x
...
"""
    The block applys temporal and spatial attention respectively.
"""
class DividedSpatialTemporalBlock(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int,
        ...
        self.window_size = window_size
        customized_components = [self.temporal_attn, self.temporal_fc]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_token = x[:, 1:, :]
        xt = rearrange(x_token, 'b (t s) m -> (b s) t m', t=self.window_size)
        res_temporal = self.drop_path1(self.temporal_attn(self.ls1(self.norm1(xt))))
        res_temporal = rearrange(res_temporal, '(b s) t m -> b (s t) m', b=B, t=self.window_size)
        res_temporal = self.temporal_fc(res_temporal)
        xt = x[:, 1:, :] + res_temporal
        
        x_cls_tokens = x_cls_token.repeat(1, self.window_size, 1)
        x_cls_tokens = rearrange(x_cls_tokens, 'b t m -> (b t) m').unsqueeze(1)
        xs = rearrange(xt, 'b (t s) m -> (b t) s m', t=self.window_size)
        xs = torch.cat((x_cls_tokens, xs), dim=1)
        res_spatial = self.drop_path2(self.attn(self.norm2(xs)))
        
        cls_token = res_spatial[:, 0, :]
        cls_token = rearrange(cls_token, '(b t) m -> b t m', t=self.window_size)
        cls_token = torch.mean(cls_token, dim=1, keepdim=True)
        res_spatial = rearrange(res_spatial[:, 1:, :], '(b t) s m -> b (s t) m', t=self.window_size)
        x = torch.cat((x_cls_token, xt), dim=1) + torch.cat((cls_token, res_spatial), dim=1)
...
class SpatialTemporalVisionTransformer(nn.Module):
    """ Vision Transformer with spatial and temporal attention
    """
    dynamic_img_size: Final[bool]
    ...
    def __init__(
            ...
            norm_layer: Optional[LayerType] = None,
            act_layer: Optional[LayerType] = None,
            block_fn: Type[nn.Module] = DividedSpatialTemporalBlock,
            mlp_layer: Type[nn.Module] = Mlp,
    ) -> None:
        """
        Args:
        ...
        num_patches = self.patch_embed.num_patches
        self.window_size = self.patch_embed.window_size
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.no_embed_class:
            ...
            sequence_pos_embed = sequence_pos_embed.repeat(1, x.size(1) // sequence_pos_embed.size(1), 1)
            sequence_pos_embed = torch.cat([cls_pos_embed, sequence_pos_embed], dim=1)
            
            
            if to_cat:
                x = torch.cat(to_cat + [x], dim=1)
            # x = x + pos_embed
            x = x + sequence_pos_embed
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        x = self.patch_embed(x)
        window_size = x.size(1) // (self.pos_embed.size(1) - 1)
        x = self._pos_embed(x)
...
class WindowVisionTransformer(nn.Module):
    ...
    def init_weights(self, mode: Literal['jax', 'jax_nlhb', 'moco', ''] = '') -> None:
        ...
        head_bias = -math.log(self.num_classes) if 'nlhb' in mode else 0.
        trunc_normal_(self.pos_embed, std=.02)
        trunc_normal_(self.window_pos_embed, std=.02)
        if self.cls_token is not None:
            nn.init.normal_(self.cls_token, std=1e-6)
        named_apply(get_init_weights_vit(mode, head_bias), self)
    ...
    @torch.jit.ignore
    def no_weight_decay(self) -> Set:
        return {'pos_embed', 'window_pos_embed', 'cls_token', 'dist_token'}
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        # use window_pos_embed instead of pos_embed
        if self.dynamic_img_size:
            B, H, W, C = x.shape
            pos_embed = resample_abs_pos_embed(
...
class WindowVisionTransformerSeparateWithTemporalToken(nn.Module):
    """ Window Vision Transformer
    ...
    def __init__(
            ...
            norm_layer: Optional[LayerType] = None,
            act_layer: Optional[LayerType] = None,
            block_fn: Type[nn.Module] = SeparateTemporalBlock,
            mlp_layer: Type[nn.Module] = Mlp,
            **kwargs
    ) -> None:
        """
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if class_token else None
        self.temporal_token = nn.Parameter(torch.zeros(1, self.window_size, embed_dim))
        self.reg_token = nn.Parameter(torch.zeros(1, reg_tokens, embed_dim)) if reg_tokens else None
        pos_embed_len = 197
        embed_len = num_patches // self.window_size if no_embed_class else num_patches // self.window_size + self.num_prefix_tokens
        self.pos_embed = nn.Parameter(torch.randn(1, pos_embed_len, embed_dim) * .02)    # divide the pos_embed to the original dimension for pretrained weights
    ...
    def init_weights(self, mode: Literal['jax', 'jax_nlhb', 'moco', ''] = '') -> None:
        ...
        if self.cls_token is not None:
            nn.init.normal_(self.cls_token, std=1e-6)
        nn.init.normal_(self.temporal_token, std=1e-6)
        named_apply(get_init_weights_vit(mode, head_bias), self)
    ...
    def _pos_embed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.no_embed_class:
            ...
            if to_cat:
                ...
                for i in range(self.window_size):
                    temporal_sequence_to_cat.append(self.temporal_token[:, i, :].unsqueeze(1).expand(x.shape[0], -1, -1))
    ...
    def _time_embed(self, x: torch.Tensor, window_size: int) -> torch.Tensor:
        ...
        temporal_token = x[:, 0, :].unsqueeze(1)
        sequence_token = x[:, 1:, :]
        temporal_token = rearrange(temporal_token, '(b w) t m -> b (w t) m', w=window_size)
...
class WindowVisionTransformerSeparateWithTemporalTokenAlpha(nn.Module):
    """ Window Vision Transformer
    ...
    def __init__(
            ...
            norm_layer: Optional[LayerType] = None,
            act_layer: Optional[LayerType] = None,
            block_fn: Type[nn.Module] = SeparateTemporalBlockAlpha,
            mlp_layer: Type[nn.Module] = Mlp,
            **kwargs
    ) -> None:
        """
...
class WindowVisionTransformerSeparatePosEmbed(nn.Module):
    """ Window Vision Transformer
    ...
    def __init__(
        ...
        self.reg_token = nn.Parameter(torch.zeros(1, reg_tokens, embed_dim)) if reg_tokens else None
        embed_len = num_patches // self.window_size if no_embed_class else num_patches // self.window_size + self.num_prefix_tokens
        self.pos_embed = nn.Parameter(torch.randn(1, embed_len, embed_dim) * .02)    # divide the pos_embed to the original dimension for pretrained weights
        self.pos_drop = nn.Dropout(p=pos_drop_rate)
        self.time_embed = nn.Parameter(torch.randn(1, self.window_size, embed_dim)* .02)
        self.time_drop = nn.Dropout(p=time_drop_rate)
        if patch_drop_rate > 0:
...
def _create_window_vision_transformer(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
...
def _create_window_vision_transformer_separate_embed(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    return build_model_with_cfg(
        WindowVisionTransformerSeparatePosEmbed,
        variant,
        pretrained,
        pretrained_filter_fn=_filter_fn,
        pretrained_strict=strict,
...
def _create_window_vision_transformer_separate_with_temporal_token(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    return build_model_with_cfg(
        WindowVisionTransformerSeparateWithTemporalToken,
        variant,
        pretrained,
        pretrained_filter_fn=_filter_fn,
        pretrained_strict=strict,
...
def _create_window_vision_transformer_separate_with_temporal_token_alpha(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    return build_model_with_cfg(
        WindowVisionTransformerSeparateWithTemporalTokenAlpha,
        variant,
        pretrained,
        pretrained_filter_fn=_filter_fn,
        pretrained_strict=strict,
...
def _create_divided_spatial_temporal_vision_transformer(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    return build_model_with_cfg(
        SpatialTemporalVisionTransformer,
        variant,
        pretrained,
        pretrained_filter_fn=_filter_fn,
        pretrained_strict=strict,
...
def _create_divided_spatial_temporal_window_vision_transformer(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer:
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
